...
| Hot user patching | Code path | `ApplyConfig` returns `updated` for user-only changes and replaces protocol user tables in-place. Native `ApplyUserDelta` applies per-inbound added/updated/deleted/full users with optional revision checks and no listener rebind. Deleted users fail new authentication immediately; common async TCP relays using the shared user bandwidth limiter exit when revocation is observed even while idle, HY2/TUIC QUIC accept/datagram/TCP relay loops now also watch limiter revocation, TLS/Vision/WebSocket/Shadowsocks relays apply revocation on upload and download checkpoints, and deleted/full-snapshot removals clear active device-session counters so re-added users are not blocked by stale idle sessions. There is still no central socket-registry hard close yet. Captured `user_id` values remain available for tail traffic. |

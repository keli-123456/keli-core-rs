...
| Hysteria2 | Rendered by `kelinode-rs` | Partial | QUIC listener, password auth, TCP relay, UDP relay, salamander obfs, bandwidth options, per-user traffic, speed/device limits, ApplyUserDelta auth plus TCP relay data-path coverage | Real-client matrix and production soak |

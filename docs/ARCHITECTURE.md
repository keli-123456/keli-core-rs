...
`ApplyUserDelta` is the native small-change path. It applies added, updated, deleted, or full-snapshot users to one inbound without rebinding the listener. The core tracks an optional revision per inbound: non-full deltas with a known `base_revision` must match the current revision, while a full snapshot may reset the revision after a mismatch. Deleting a user immediately rejects new authentication for that user. Accepted TCP connections on SOCKS, HTTP CONNECT/plain HTTP, VLESS, VMess, Trojan, and Shadowsocks main relay paths are registered by user and actively shut down on delete/full-snapshot removal; common async TCP relays that hold the shared user bandwidth limiter also exit when revocation is observed even while idle, HY2/TUIC QUIC accept/datagram/TCP relay loops watch limiter revocation, TLS/Vision/WebSocket/Shadowsocks relays apply revocation on both upload and download checkpoints, deleted/full-snapshot removals clear active device-session counters to avoid stale idle sessions after re-add, and already captured `user_id` values remain available for tail traffic accounting.
